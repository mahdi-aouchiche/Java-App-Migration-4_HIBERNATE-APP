 ...
 */
@WebServlet("/view-non-affiliated-employees")
public class ViewNonAffiliatedEmployees extends HttpServlet {
		...
		throws ServletException, IOException
	{
		...
		request.setAttribute("employeeList", employeesAssociatedToDepartment);
		request.setAttribute("returnURL", request.getServletPath());
		request.setAttribute("informationType", message);
		request.getRequestDispatcher("WEB-INF/views/view-edit-employees.jsp").forward(request, response);			
	}
